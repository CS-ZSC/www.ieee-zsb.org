...
function CardHorizontal({
  ...
  member: (typeof executiveBoard)[0];
}) {
  return (
    <Flex
      direction="row"
    ...
    <Flex flexWrap="wrap" gap="10px" width="100%" justifyContent="center">
      {executiveBoard.map((member, i) => (
        <Box
          key={i}
          flex={
            isDesktop ? "1 1 calc(33.333% - 7px)" : "0 1 calc(50% - 5px)"
          }
          maxWidth={
            isDesktop ? "calc(33.333% - 7px)" : "calc(50% - 5px)"
          }
        >
          <Link href={`/about/member/${getSlug(member.name)}`} style={{ textDecoration: "none" }}>
            {isDesktop ? (
